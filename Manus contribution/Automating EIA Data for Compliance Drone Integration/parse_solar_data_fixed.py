...
def parse_solar_data():
    ...
    try:
        # Read the Excel file with proper header row (row 2, which is index 1)
        df = pd.read_excel("/home/ubuntu/upload/USSolarProjectsfor2025andbeyond.xlsx", header=2)
        ...
        if len(solar_df) > 0:
            ...
            sample_cols = ['Entity Name', 'Plant Name', 'Plant State', 'Technology', 
                          'Nameplate Capacity (MW)', 'Operating Year', 'Status', 
                          'Latitude', 'Longitude']
            print(solar_df[sample_cols].head(10))
            ...
            summary = {
                ...
                "capacity_stats": {
                    "total_mw": float(solar_df['Nameplate Capacity (MW)'].sum()),
                    "average_mw": float(solar_df['Nameplate Capacity (MW)'].mean()),
                    "max_mw": float(solar_df['Nameplate Capacity (MW)'].max()),
                    "min_mw": float(solar_df['Nameplate Capacity (MW)'].min())
                }
            }
        ...
        if len(future_solar) > 0:
            ...
            print("\n=== FUTURE SOLAR BY STATE ===")
            future_state_counts = future_solar['Plant State'].value_counts()
            print(future_state_counts.head(10))
